class CreateSchoolYearlyStats < ActiveRecord::Migration[5.0]
  def change
    create_table :school_yearly_stats do |t|
      # Alternate composite key on school NCES ID and dataset year
      t.references :school, null: false, index: false, comment: 'NCES public school ID'
      ...
      # Index on alternate composite key, school ID first
      t.index [:school_id, :school_year], unique: true
    end
  end
end
